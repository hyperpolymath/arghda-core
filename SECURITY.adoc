...
This policy covers the arghda-core engine. arghda reads `+.agda+` files
as text and shells out to the configured `+agda+` (and, in future,
`+agda-unused+`) binary on files you point it at; it never executes
proof code itself. Reports about untrusted-input handling — path
traversal, command construction, denial of service on malformed input —
